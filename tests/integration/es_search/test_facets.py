...
class FacetTestCase(TestCase):
    ...
    def test_build_facet_return_value(self):
        facet = Facet('agg_name', {}, 'http://example.com', {})
        self.assertEqual(
            {
                'name': 'Yes',
                'count': 12,
    ...
    def test_get_display_name_uses_label_defined_in_settings(self):
        facet = Facet('agg_name', {}, 'http://example.com', {'agg_name': {'label': 'Agg name'}})
...
class RangeFacetTestCase(TestCase):
    ...
    def test_empty_list_returned_if_all_buckets_have_doc_count_of_zero(self):
        ...
        facet = RangeFacet('agg_name', {'buckets': buckets}, 'http://example.com', {})
...
class HistogramFacetTestCase(TestCase):
    ...
    def test_get_display_name_with_zero_key(self):
        facet = HistogramFacet('agg_name', {}, 'http://example.com',
                               {'agg_name': {'label': 'Agg name', 'params': {'field': 'agg_field', 'interval': 10}}})
    ...
    def test_get_display_name_with_key_and_interval(self):
        facet = HistogramFacet('agg_name', {}, 'http://example.com',
                               {'agg_name': {'label': 'Agg name', 'params': {'field': 'agg_field', 'interval': 40}}})
...
class FacetsBuilderTestCase(TestCase):
    ...
    def test_class_uses_correct_facet_type_class(self):
        self.assertIs(
            FacetsBuilder({}, '').get_facet_class('range'),
            RangeFacet
        )

        self.assertIs(
            FacetsBuilder({}, '').get_facet_class('auto_range'),
            RangeFacet
        )

        self.assertIs(
            FacetsBuilder({}, '').get_facet_class('histogram'),
            HistogramFacet
        )

    def test_get_facet_class_defaults_to_Facet_if_right_class_not_found(self):
        self.assertIs(
            FacetsBuilder({}, '').get_facet_class('term'),
            Facet
        )

    def test_sort_facets_sorts_facets_according_to_user_defined_facets_key_order(self):
        user_defined_facets = OrderedDict([
            ('first_facet', {}),
            ('second_facet', {})
        ])

        builder = FacetsBuilder({}, '', user_defined_facets=user_defined_facets)
    ...
    def test_build_facet_return_value(self):
        builder = FacetsBuilder(
            ...
            user_defined_facets={'agg_name': {'type': 'term'}}
        )
