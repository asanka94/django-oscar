...
class BaseSearch(FacetedSearch):
    ...
    def __init__(self, search_filters=None, page_number=1, max_results=0, selected_facets=None, *args, **kwargs):
        ...
        self.search_settings = settings.OSCAR_SEARCH['SEARCHES'][self.settings_key]
        self.fields = self.search_settings.get('query', {}).get('fields', [])
        self.auto_ranges = {}

        # This setting turns on/off faceting.
        self.aggregate_search = self.search_settings.get('aggregate_search', False)
    ...
    def get_main_facets(self):
        """
        Iterate over this classes's defined `facets` setting and generate each facet's respective Facet instance.
        """
        facets = {}
        facet_data = self.search_settings.get('facets', {})
    ...
    def get_extra_facets(self):
        """
        Similar to `get_main_facets` but the facets are not defined in the `facets` setting. This can be used to add one
        of facets in a subclass.
        """
        return {}
    ...
    def query(self, search, query):
        ...
        if query:
            query_dict = self.search_settings['query'].copy()
